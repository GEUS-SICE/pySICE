...
c***********************************************
     ...
     c  alam,reflec,height,aot,anna,pi,tauaer,
...
c     main output after postprocessing
       ...
       open(7001,file= 'output_spectral_spherical_albedo_solved.dat')
...
c     imaginary part of ice refractive index at 865 and 1020nm      
       alt3=4.*pi*2.40e-7/0.865
       alt4=4.*pi*2.25e-6/1.02
...
c     derivation of effective absorption length - dlina (mm):
      dmkm=alog(ctoa(21)/r0)*alog(ctoa(21)/r0)/AB1020/tuzik**2.
      dlina=1.e-3*dmkm   
...
c!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!         
c                                         STEP 5
c                  study of quality of retrievals and print of retrieved 
c                                     snow characteristics         
 ...
 202    continue
        ...
        NBARE=0
        NSNOW=0
c       dark ice, clean ice and snow index        
...
c     snow cover fraction, diameter of grains, SSA, EAL,R0,        
...
c                     retrieved TOC,ECMWF TOC, difference(%),cv1,cv2
        WRITE(1004,*)
     c  j,alat,alon, NCLASS,factor,diam,ssa,dlina,r0, aload1,powe,polut,
     c  deff,absor1,absef660,absor1000, rsw, rvis,rnir,
     c  rsws, rviss,rnirs, andbi,andsi,ratka, NPOLE, NBARE, NSNOW,
     c sza,vza,raa,toa(1),toa(21), tocos,akozon,difka,cv1,cv2
...
c        if (ccv2.gt.THV1.or.diam.lt.THV2.or.DIFKA.gt.THV3) go to 932
        
c     *****    output: postprocessing *****
      WRITE(7004,*) j,alat,alon,   
     c NCLASS,factor,diam,ssa,dlina,r0, aload1,powe,polut,
     c deff,absor1,absef660,absor1000,rsw,rvis,rnir,rsws,rviss,rnirs,
     c ansi,andbi,ratka,npole,nbare,nsnow, sza,vza,raa,toa(1),toa(21),
...
c               MINIMIZATION FUNCTION:      
      FUNCTION  fun(x)
       common sza,vza,raa, am1,am2,u1,u2,co,alam,reflec,
     c  height,aot,anna,pi,tauaer, taumol,gaer,t620
           tauaer=aot*(alam/0.5)**(-anna)
             taumol=0.008735/alam**(4.08)
                   okota=height/6000.
                   if (okota.gt.0.) taumol=taumol*exp(-okota)                    
