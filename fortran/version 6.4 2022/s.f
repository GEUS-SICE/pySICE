...
c                  USING OLCI  / S-3ABCD   
c                  a.a.kokhanovsky@gmail.com
c                  October 1, 2022
c                  Version 6.4 (search: bav 2022), with corrections of B. Vandecrux
c***********************************************
...
c          WRITE(*,*) r0,ctoa(1),ctoa(2),eps,toa(1),toa(2),factor,
c     c      bt,bt17,psi
          refla=r0
          rv=r0
      akapf=2.25e-6
...
c     the mininimization routine has been finished
 ...
 6363 continue
c       WRITE(*,*) (toa(ns),ns=1,21)
c       WRITE(*,*) (answer(NB),NB=1,21)
...
c           write(*, *) x,fun,reflec,rfinal,refatm,tatm,r0,x,z,albatm
c**********************************                    
                                             return  
                                             end
...
c     QUADRATIC POLYNOMIAL for the range below 709nm
