...
input_file = pd.read_csv(
    OutputFolder + "input.dat", delim_whitespace=True, header=None
).values
...
for var in var_list:
    ...
    if var == "isnow":
        ...
        cmap, norm = from_levels_and_colors(
            vals, [cmap(x) for x in np.linspace(0, 1, len(vals) - 1)]
        )
    ...
    (ds_p - ds_f).dropna("x", "all").dropna("y", "all").plot(
        ax=ax[2], cbar_kwargs={"label": code_ver2 + " - " + code_ver1}
    )
    ax[2].set_title(code_ver2 + " - " + code_ver1)
    ds_f = ds_f.where(ds_p.notnull())
    ds_p = ds_p.where(ds_f.notnull())
    ax[3].plot(
        ds_f.where(isnow == 1).values.flatten(),
    ...
    ax[3].plot(
        ds_f.where(isnow == 2).values.flatten(),
        ds_p.where(isnow == 2).values.flatten(),
        marker=".",
        linestyle="None",
        label="polluted pixels",
    )
    ax[3].plot(
        ds_f.where(isnow == 3).values.flatten(),
