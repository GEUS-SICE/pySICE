...
tmp = bl.heatmap_discrete(
    rio.open("out/" + InputFolder_1 + "diagnostic_retrieval.tif").read(1),
    "isnow_py " + InputFolder_1,
)
try:
    tmp = bl.heatmap_discrete(
        rio.open("out/" + InputFolder_2 + "diagnostic_retrieval.tif").read(1),
...
plt.figure()
tmp = bl.heatmap(
    rio.open("out/" + InputFolder_1 + "albedo_bb_planar_sw.tif").read(1),
    "albedo_bb_planar_sw",
)
...
for i in range(len(path_files)):
    ...
    ax.set_title(
        "diagnostic_retrieval   " + path_files[i][105 : len(path_files[i]) - 25]
    )
    plt.savefig(
        path_files[i][0 : len(path_files[i]) - 25] + "/plots/diagnostic_retrieval.png",
        bbox_inches="tight",
    )
    plt.close()
