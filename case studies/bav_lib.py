...
def stat_title(x, y, ax):
    ...
    ax.set_title(
        "R2=%.3f\nRMSE=%.2f\nN=%.0f"
        % (r2_score(y, preds), mean_squared_error(y, preds), len(x))
    )
    return ax
...
def multi_plot(
    ...
    f1.subplots_adjust(
        hspace=0.2, wspace=0.1, left=0.08, right=0.95, bottom=0.2, top=0.9
    )
    count = -1
    for site in sites:
        print(site)
...
def heatmap(var, title="", col_lim=(np.nan, np.nan), cmap_in="gnuplot"):
    ...
    #    fig,ax = plt.subplots()
    im = plt.imshow(
        z, cmap=cmap, interpolation="nearest", vmin=col_lim[0], vmax=col_lim[1]
    )
    cb = plt.colorbar(im)
...
def heatmap_discrete(var, title="", col_lim=(np.nan, np.nan), cmap_in="gnuplot"):
    ...
    tic_locs = (
        bounds[0 : len(bounds) - 1]
        - (bounds[0 : len(bounds) - 1] - bounds[1 : len(bounds)]) / 2
    )
...
def mosaic_albedo_fit(df, Rad_in):
    ...
    if Rad_in == "Rt":
        ...
        fig.text(
            0.5, 0.05, "Bottom of atmosphere OLCI reflectance", ha="center", size=20
        )
    ...
    for i in range(7):
        for j in range(3):
            ...
            ax[i, j].text(
                ...
                0.5,
                "R2=%.3f\nRMSE=%.2f\nN=%.0f"
                % (r2_score(alb, preds), mean_squared_error(alb, preds), len(R)),
                fontsize=13,
                color="white",
                fontweight="bold",
                horizontalalignment="left",
