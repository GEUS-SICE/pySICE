...
def polar_stere(lon_w, lon_e, lat_s, lat_n, **kwargs):
    ...
    **kwargs -- Aditional arguments for Basemap object.
    ...
    """
    lon_0 = lon_w + (lon_e - lon_w) / 2.0
    ref = lat_s if abs(lat_s) > abs(lat_n) else lat_n
    lat_0 = math.copysign(90.0, ref)
    proj = "npstere" if lat_0 > 0 else "spstere"
    prj = Basemap(
        projection=proj, lon_0=lon_0, lat_0=lat_0, boundinglat=0, resolution="c"
    )
...
for ii in range(0, lats.shape[0], 50):
    ...
    for jj in range(0, lats.shape[1], 50):
        if lats[ii, jj] > 59 and lats[ii, jj] < 84 and alt[ii, jj] > 1:
            #            and lons[ii,jj]< -29 \
            #            and lons[ii,jj]>-57 :
