...
plt.figure()
(OLCI_scene.toa.sel(band=0) - OLCI_scene.toa_cor.sel(band=0)).unstack(dim='xy').transpose('y', 'x').plot()
...
import xarray as xr
plt.close('all')
for var in ['grain_diameter', 'albedo_bb_planar_sw', 'albedo_bb_spherical_sw', 'snow_specific_area']:
